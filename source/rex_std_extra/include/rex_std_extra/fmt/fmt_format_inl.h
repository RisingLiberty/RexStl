...
#include <rex_stl/fmt/fmt_format.h>

FMT_BEGIN_NAMESPACE
namespace detail
{

  FMT_FUNC void assert_fail(const char* file, int32 line, const char* message)
  {
  ...
  FMT_FUNC void format_error_code(detail::buffer<char>& out, int32 error_code, string_view message) noexcept
  {
  ...
  FMT_FUNC void report_error(format_func func, int32 error_code, const char* message) noexcept
  {
  ...
  {
    auto& facet        = rsl::use_facet<std::numpunct<Char>>(loc.get<std::locale>());
  ...
  {
    return rsl::use_facet<std::numpunct<Char>>(loc.get<std::locale>()).decimal_point();
  }
#else
  template <typename Char>
...
FMT_FUNC std::system_error vsystem_error(int32 error_code, string_view format_str, format_args args)
{
...
namespace detail
{
  ...
  struct bits
  {
    static FMT_CONSTEXPR_DECL const int32 value = static_cast<int32>(sizeof(T) * rsl::NumericLimits<unsigned char>::digits);
  };
  ...
  struct basic_fp
  {
    ...
    static constexpr const int32 num_significand_bits = bits<F>::value;
    ...
    constexpr basic_fp(uint64_t f_val, int32 e_val)
        : f(f_val)
        , e(e_val)
    {
    }
    ...
    {
      static_assert((rsl::NumericLimits<Float>::is_iec559 && rsl::NumericLimits<Float>::digits <= 113) || is_float128<Float>::value, "unsupported FP");
      ...
      auto biased_e                         = static_cast<int32>((u & exponent_mask<Float>()) >> num_float_significand_bits);
  ...
  template <int32 SHIFT = 0, typename F>
  ...
  FMT_CONSTEXPR inline fp get_cached_power(int32 min_exponent, int32& pow10_exponent)
  {
    const int32 shift = 32;
    // log10(2) = 0x0.4d104d427de7fbcc...
    const int64_t significand = 0x4d104d427de7fbcc;
    int32 index               = static_cast<int32>(((min_exponent + fp::num_significand_bits - 1) * (significand >> shift) + ((int64_t(1) << shift) - 1)) // ceil
                                     >> 32                                                                                                  // arithmetic shift
    );
    // Decimal exponent of the first (smallest) cached power of 10.
    const int32 first_dec_exp = -348;
    // Difference between 2 consecutive decimal exponents in cached powers of 10.
    const int32 dec_exp_step = 8;
  ...
  private:
    ...
    FMT_CONSTEXPR20 bigit operator[](int32 index) const
    {
      return bigits_[to_unsigned(index)];
    }
    FMT_CONSTEXPR20 bigit& operator[](int32 index)
    {
      return bigits_[to_unsigned(index)];
    }

    static FMT_CONSTEXPR_DECL const int32 bigit_bits = bits<bigit>::value;

    friend struct formatter<bigint>;

    FMT_CONSTEXPR20 void subtract_bigits(int32 index, bigit other, bigit& borrow)
    {
    ...
    {
      int32 num_bigits = static_cast<int32>(bigits_.size()) - 1;
    ...
    FMT_CONSTEXPR20 void multiply(UInt value)
    {
      ...
      const int32 shift = num_bits<half_uint>() - bigit_bits;
  ...
  public:
    ...
    FMT_CONSTEXPR20 int32 num_bigits() const
    {
      return static_cast<int32>(bigits_.size()) + exp_;
    }

    FMT_NOINLINE FMT_CONSTEXPR20 bigint& operator<<=(int32 shift)
    {
      FMT_ASSERT(shift >= 0, "");
    ...
    friend FMT_CONSTEXPR20 int32 compare(const bigint& lhs, const bigint& rhs)
    {
      int32 num_lhs_bigits = lhs.num_bigits(), num_rhs_bigits = rhs.num_bigits();
      if(num_lhs_bigits != num_rhs_bigits)
        return num_lhs_bigits > num_rhs_bigits ? 1 : -1;
      int32 i   = static_cast<int32>(lhs.bigits_.size()) - 1;
      int32 j   = static_cast<int32>(rhs.bigits_.size()) - 1;
      int32 end = i - j;
      if(end < 0)
        end = 0;
      for(; i >= end; --i, --j)
    ...
    friend FMT_CONSTEXPR20 int32 add_compare(const bigint& lhs1, const bigint& lhs2, const bigint& rhs)
    {
      int32 max_lhs_bigits = (std::max)(lhs1.num_bigits(), lhs2.num_bigits());
      int32 num_rhs_bigits = rhs.num_bigits();
      if(max_lhs_bigits + 1 < num_rhs_bigits)
        return -1;
      if(max_lhs_bigits > num_rhs_bigits)
        return 1;
      auto get_bigit      = [](const bigint& n, int32 i) -> bigit { return i >= n.exp_ && i < n.num_bigits() ? n[i - n.exp_] : 0; };
      double_bigit borrow = 0;
      int32 min_exp       = (std::min)((std::min)(lhs1.exp_, lhs2.exp_), rhs.exp_);
      for(int32 i = num_rhs_bigits - 1; i >= min_exp; --i)
      {
    ...
    FMT_CONSTEXPR20 void assign_pow10(int32 exp)
    {
      FMT_ASSERT(exp >= 0, "");
      if(exp == 0)
        return *this = 1;
    ...
    {
      int32 num_bigits        = static_cast<int32>(bigits_.size());
      int32 num_result_bigits = 2 * num_bigits;
      basic_memory_buffer<bigit, bigits_capacity> n(std::move(bigits_));
      bigits_.resize(to_unsigned(num_result_bigits));
      auto sum = uint128_t();
      for(int32 bigit_index = 0; bigit_index < num_bigits; ++bigit_index)
      {
        // Compute bigit at position bigit_index of the result by adding
        // cross-product terms n[i] * n[j] such that i + j == bigit_index.
        for(int32 i = 0, j = bigit_index; j >= 0; ++i, --j)
        {
      ...
      for(int32 bigit_index = num_bigits; bigit_index < num_result_bigits; ++bigit_index)
      {
        for(int32 j = num_bigits - 1, i = bigit_index - j; i < num_bigits;)
          sum += static_cast<double_bigit>(n[i++]) * n[j--];
        (*this)[bigit_index] = static_cast<bigit>(sum);
    ...
    {
      int32 exp_difference = exp_ - other.exp_;
      if(exp_difference <= 0)
        return;
      int32 num_bigits = static_cast<int32>(bigits_.size());
      bigits_.resize(to_unsigned(num_bigits + exp_difference));
      for(int32 i = num_bigits - 1, j = i + exp_difference; i >= 0; --i, --j)
        bigits_[j] = bigits_[i];
    ...
    FMT_CONSTEXPR20 int32 divmod_assign(const bigint& divisor)
    {
  ...
  struct gen_digits_handler
  {
    ...
    FMT_CONSTEXPR digits::result on_digit(char digit, uint64_t divisor, uint64_t remainder, uint64_t error, bool integral)
    {
      ...
      ++buf[size - 1];
      for(int32 i = size - 1; i > 0 && buf[i] > '9'; --i)
      {
        buf[i] = '0';
        ++buf[i - 1];
      }
  ...
  inline FMT_CONSTEXPR20 void adjust_precision(int32& precision, int32 exp10)
  {
    // Adjust fixed precision by exponent because it is relative to decimal
    // point.
    if(exp10 > 0 && precision > max_value<int32>() - exp10)
  ...
  FMT_INLINE FMT_CONSTEXPR20 digits::result grisu_gen_digits(fp value, uint64_t error, int32& exp, gen_digits_handler& handler)
  {
    ...
    do
    {
      ...
      auto result    = handler.on_digit(static_cast<char>('0' + digit), impl_data::power_of_10_64[exp] << -one.e, remainder, error, true);
  ...
  inline FMT_CONSTEXPR20 uint128_fallback& uint128_fallback::operator+=(uint64_t n) noexcept
  {
  ...
  namespace dragonbox
  {
    ...
    struct cache_accessor<float>
    {
      ...
      static compute_mul_parity_result compute_mul_parity(carrier_uint two_f, const cache_entry_type& cache, int32 beta) noexcept
      {
        FMT_ASSERT(beta >= 1, "");
        FMT_ASSERT(beta < 64, "");
    ...
    template <class T>
    bool is_left_endpoint_integer_shorter_interval(int32 exponent) noexcept
    {
    ...
    template <class T>
    FMT_INLINE decimal_fp<T> shorter_interval_case(int32 exponent) noexcept
    {
    ...
    decimal_fp<T> to_decimal(T x) noexcept
    {
      ...
      else
      {
        ...
        if(significand == 0)
          return {0, 0};
        exponent = rsl::NumericLimits<T>::min_exponent - num_significand_bits<T>() - 1;
      }
      ...
      else
      {
        ...
        if(!include_left_endpoint || exponent < float_info<T>::case_fc_pm_half_lower_threshold || exponent > float_info<T>::divisibility_check_by_5_threshold)
        {
        ...
        {
          const typename cache_accessor<T>::compute_mul_parity_result x_mul = cache_accessor<T>::compute_mul_parity(two_fl, cache, beta);
  ...
  FMT_CONSTEXPR20 inline void format_dragon(basic_fp<uint128_t> value, unsigned flags, int32 num_digits, buffer<char>& buf, int32& exp10)
  {
    ...
    int32 shift                = is_predecessor_closer ? 2 : 1;
    if(value.e >= 0)
    {
      numerator = value.f;
    ...
    if(num_digits < 0)
    {
      ...
      for(;;)
      {
        int32 digit = numerator.divmod_assign(denominator);
        ...
        if(low || high)
        {
          ...
          {
            int32 result = add_compare(numerator, numerator, denominator);
            // Round half to even.
    ...
    buf.try_resize(to_unsigned(num_digits));
    for(int32 i = 0; i < num_digits - 1; ++i)
    {
      int32 digit = numerator.divmod_assign(denominator);
    ...
    if(result > 0 || (result == 0 && (digit % 2) != 0))
    {
      if(digit == 9)
      {
        ...
        // Propagate the carry.
        for(int32 i = num_digits - 1; i > 0 && buf[i] == overflow; --i)
        {
          buf[i] = '0';
          ++buf[i - 1];
        }
...
#ifdef _MSC_VER
  FMT_FUNC auto fmt_snprintf(char* buf, size_t size, const char* fmt, ...) -> int32
  {
    auto args = va_list();
    va_start(args, fmt);
  ...
  template <typename Float>
  FMT_HEADER_ONLY_CONSTEXPR20 int32 format_float(Float value, int32 precision, float_specs specs, buffer<char>& buf)
  {
    ...
    if(!is_fast_float<Float>())
    {
      ...
      exp          = static_cast<int32>(std::ceil((f.e + count_digits<1>(f.f) - 1) * inv_log2_10 - 1e-10));
    ...
    else
    {
      ...
      const int32 min_exp   = -60; // alpha in Grisu.
      int32 cached_exp10    = 0;   // K in Grisu.
    ...
    if(use_dragon)
    {
      ...
      const int32 max_double_digits = 767;
...
FMT_FUNC void format_system_error(detail::buffer<char>& out, int32 error_code, const char* message) noexcept
{
  FMT_TRY
  {
    auto ec = std::error_code(error_code, std::generic_category());
...
FMT_FUNC void report_system_error(int32 error_code, const char* message) noexcept
{
