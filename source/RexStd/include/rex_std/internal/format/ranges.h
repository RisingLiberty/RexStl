...
namespace detail
{
  ...
  // Returns true if T has a rsl::string-like interface, like rsl::string_view.
  template <typename T>
  ...
  public:
    static constexpr const bool value = is_string<T>::value || rsl::is_convertible<T, std_string_view<char>>::value || !rsl::is_void<decltype(check<T>(nullptr))>::value;
  };

  template <typename Char>
  struct is_std_string_like<rsl::basic_string_view<Char>> : rsl::true_type
  {
  };
...
#else
    static constexpr const bool value = !rsl::is_void<decltype(check<T>(nullptr))>::value;
#endif
  };
  ...
  public:
    ...
    static constexpr const bool value = false;
#else
    static constexpr const bool value = !rsl::is_void<decltype(check<T>(nullptr))>::value && !is_map<T>::value;
#endif
  };
  ...
  struct is_range_ : rsl::false_type
  {
  };
  ...
  struct has_member_fn_begin_end_t : rsl::false_type
  {
  };

  template <typename T>
  struct has_member_fn_begin_end_t<T, void_t<decltype(rsl::declval<T>().begin()), decltype(rsl::declval<T>().end())>> : rsl::true_type
  {
  };
  ...
  template <typename T, typename Enable = void>
  struct has_const_begin_end : rsl::false_type
  {
  };
  template <typename T, typename Enable = void>
  struct has_mutable_begin_end : rsl::false_type
  {
  };

  template <typename T>
  struct has_const_begin_end<T, void_t<decltype(detail::range_begin(rsl::declval<const remove_cvref_t<T>&>())), decltype(detail::range_end(rsl::declval<const remove_cvref_t<T>&>()))>> : rsl::true_type
  {
  };

  template <typename T>
  struct has_mutable_begin_end<T, void_t<decltype(detail::range_begin(rsl::declval<T>())), decltype(detail::range_end(rsl::declval<T>())), enable_if_t<rsl::is_copy_constructible<T>::value>>> : rsl::true_type
  {
  };
  ...
  {
    template <typename U>
    static auto check(U* p) -> decltype(rsl::tuple_size<U>::value, int());
    template <typename>
    static void check(...);
...
// Check for integer_sequence
  ...
  using integer_sequence = rsl::integer_sequence<T, N...>;
  template <size_t... N>
  using index_sequence = rsl::index_sequence<N...>;
  template <size_t N>
  using make_index_sequence = rsl::make_index_sequence<N>;
#else
  template <typename T, T... N>
  ...
  template <typename T>
  using tuple_index_sequence = make_index_sequence<rsl::tuple_size<T>::value>;
  ...
  class is_tuple_formattable_<T, C, true>
  {
    ...
    static rsl::true_type check2(index_sequence<I...>, integer_sequence<bool, (I == I)...>);
    static rsl::false_type check2(...);
    template <rsl::size_t... I>
    static decltype(check2(index_sequence<I...> {}, integer_sequence<bool, (is_formattable<typename rsl::tuple_element<I, T>::type, C>::value)...> {})) check(index_sequence<I...>);
  ...
  void for_each(Tuple&& tup, F&& f)
  {
    ...
    for_each(indexes, rsl::forward<Tuple>(tup), rsl::forward<F>(f));
  }
  ...
  {
    using type = decltype(*detail::range_begin(rsl::declval<R&>()));
  };
  ...
  using range_reference_type = decltype(*detail::range_begin(rsl::declval<Range&>()));
#endif
  ...
  template <typename Range>
  using uncvref_first_type = remove_cvref_t<decltype(rsl::declval<range_reference_type<Range>>().first)>;

  template <typename Range>
  using uncvref_second_type = remove_cvref_t<decltype(rsl::declval<range_reference_type<Range>>().second)>;
  ...
  template <typename Char, typename OutputIt, typename T, FMT_ENABLE_IF(rsl::is_convertible<T, std_string_view<char>>::value)>
  ...
  template <typename Char, typename OutputIt, typename Arg, FMT_ENABLE_IF(rsl::is_same<Arg, Char>::value)>
  OutputIt write_range_entry(OutputIt out, const Arg v)
  {
    return write_escaped_char(out, v);
  }

  template <typename Char, typename OutputIt, typename Arg, FMT_ENABLE_IF(!is_std_string_like<typename rsl::decay<Arg>::type>::value && !rsl::is_same<Arg, Char>::value)>
  OutputIt write_range_entry(OutputIt out, const Arg& v)
  {
...
struct is_range
{
  static constexpr const bool value = detail::is_range_<T>::value && !detail::is_std_string_like<T>::value && !rsl::is_convertible<T, rsl::basic_string<Char>>::value && !rsl::is_constructible<detail::std_string_view<Char>, T>::value;
};
...
template <typename T, typename Char>
struct range_formatter<T, Char, enable_if_t<conjunction<rsl::is_same<T, remove_cvref_t<T>>, disjunction<is_formattable<T, Char>, detail::has_fallback_formatter<T, Char>>>::value>>
{
private:
...
namespace detail
{
  ...
  {
    static constexpr auto value = rsl::is_same<range_reference_type<T>, T>::value ? range_format::disabled : is_map<T>::value ? range_format::map : is_set<T>::value ? range_format::set : range_format::sequence;
  };
  ...
  using range_format_constant = rsl::integral_constant<range_format, K>;
...
template <typename T, typename Char, typename Enable = void>
struct range_format_kind : conditional_t<is_range<T, Char>::value, detail::range_format_kind_<T>, rsl::integral_constant<range_format, range_format::disabled>>
{
};
...
struct formatter<tuple_join_view<Char, T...>, Char>
{
  ...
  FMT_CONSTEXPR auto parse(ParseContext& ctx) -> decltype(ctx.begin())
  {
    return do_parse(ctx, rsl::integral_constant<size_t, sizeof...(T)>());
  }

  template <typename FormatContext>
  auto format(const tuple_join_view<Char, T...>& value, FormatContext& ctx) const -> typename FormatContext::iterator
  {
    return do_format(value, ctx, rsl::integral_constant<size_t, sizeof...(T)>());
  }

private:
  rsl::tuple<formatter<typename rsl::decay<T>::type, Char>...> formatters_;

  template <typename ParseContext>
  FMT_CONSTEXPR auto do_parse(ParseContext& ctx, rsl::integral_constant<size_t, 0>) -> decltype(ctx.begin())
  {
    return ctx.begin();
  }

  template <typename ParseContext, size_t N>
  FMT_CONSTEXPR auto do_parse(ParseContext& ctx, rsl::integral_constant<size_t, N>) -> decltype(ctx.begin())
  {
    auto end = ctx.begin();
  ...
  template <typename FormatContext>
  auto do_format(const tuple_join_view<Char, T...>&, FormatContext& ctx, rsl::integral_constant<size_t, 0>) const -> typename FormatContext::iterator
  {
    return ctx.out();
  }
  ...
  auto do_format(const tuple_join_view<Char, T...>& value, FormatContext& ctx, rsl::integral_constant<size_t, N>) const -> typename FormatContext::iterator
  {
    ...
    if(N > 1)
    {
      ...
      ctx.advance_to(out);
      return do_format(value, ctx, rsl::integral_constant<size_t, N - 1>());
    }
    return out;
  }
};
 ...
 */
template <typename T>
auto join(rsl::initializer_list<T> list, string_view sep) -> join_view<const T*, const T*>
{
