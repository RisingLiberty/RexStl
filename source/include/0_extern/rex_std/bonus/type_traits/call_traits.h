...
REX_RSL_BEGIN_NAMESPACE
    ...
    namespace internal
    {
        ...
        struct call_traitsImpl2 { using param_type = const T&; };

        template <typename T>
        struct call_traitsImpl2<T, true> { using param_type = T; };

        template <typename T, bool is_pointer_v, bool B1>
        struct call_traitsImpl { using param_type = const T&; };

        template <typename T, bool is_pointer_v>
        struct call_traitsImpl<T, is_pointer_v, true> { using param_type = typename call_traitsImpl2<T, sizeof(T) <= sizeof(void*)>::param_type; };

        template <typename T, bool B1>
        struct call_traitsImpl<T, true, B1> { using param_type = const T; };
    }
