...
namespace rsl
{
    ...
    public:
        ...
        stack_string(rsl::initializer_list<const value_type> str)
            : stack_string()
        {
        ...
        {
            card32 new_length = rsl::clamp_max(view.length(), StrMaxSize);
        ...
        {
            card32 new_length = rsl::clamp_max(rsl::string_length(str), StrMaxSize);
        ...
        void assign(const value_type* new_data, card32 length)
        {
            card32 copy_size = clamp_max(length, StrMaxSize);
            rsl::memcpy(data(), new_data, copy_size);
            m_null_terminator_offset = copy_size;
        }
        ...
        auto at(card32 idx)
        {
            STATIC_WARNING("Should assert here");
        ...
        operator basic_string_view<CharType>() const
        {
            return to_view();
        }
    ...
    private:
        ...
        card32 m_null_terminator_offset;
    ...
    using Tinystack_string = stack_string<char8, 32>;
    using Smallstack_string = stack_string<char8, 64>;
    using Mediumstack_string = stack_string<char8, 128>;
    using Bigstack_string = stack_string<char8, 256>;

    Tinystack_string to_stack_string(const uint32 value);
    Tinystack_string to_stack_string(const int32 value);
    Tinystack_string to_stack_string(const uint64 value);
    Tinystack_string to_stack_string(const int64 value);
    Tinystack_string to_stack_string(const float32 value, card32 precision = 4);
    Tinystack_string to_stack_string(const float64 value, card32 precision = 4);
    Tinystack_string to_stack_string(const void* ptr);
...
namespace rsl
{
    ...
    {
        card32 operator()(const rsl::stack_string<CharType, Size>& str) const
        {
            return rsl::hash(str);
        }
    };
