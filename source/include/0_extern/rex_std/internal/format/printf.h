...
struct int_checker<true>
{
  ...
  static bool fits_in_int(T value)
  {
    return value >= (rsl::numeric_limits<int>::min)() && value <= max_value<int>();
  }
...
template <typename T>
struct make_unsigned_or_bool : rsl::make_unsigned<T>
{
};
...
void vprintf(buffer<Char>& buf, basic_string_view<Char> format, basic_format_args<Context> args)
{
  ...
  while(it != end)
  {
    ...
    if(specs.precision >= 0 && arg.type() == detail::type::cstring_type)
    {
      ...
      arg          = detail::make_arg<basic_printf_context<OutputIt, Char>>(basic_string_view<Char>(str, detail::to_unsigned(nul != str_end ? nul - str : specs.precision)));
    }
    ...
    switch(c)
    {
      ...
      case 't': convert_arg<rsl::ptrdiff_t>(arg, t); break;
      case 'L':
    ...
    if(arg.is_integral())
    {
      ...
      switch(type)
      {
        case 'i':
        case 'u': type = 'd'; break;
        case 'c': visit_format_arg(detail::char_converter<basic_printf_context<OutputIt, Char>>(arg), arg); break;
      }
    }
...
inline auto vsprintf(const S& fmt, basic_format_args<basic_printf_context_t<type_identity_t<Char>>> args) -> rsl::basic_string<Char>
{
...
inline auto sprintf(const S& fmt, const T&... args) -> rsl::basic_string<Char>
{
  ...
  return vsprintf(detail::to_string_view(fmt), rsl::make_format_args<context>(args...));
}

template <typename S, typename Char = char_t<S>>
inline auto vfprintf(rsl::FILE* f, const S& fmt, basic_format_args<basic_printf_context_t<type_identity_t<Char>>> args) -> int
{
  basic_memory_buffer<Char> buffer;
  vprintf(buffer, detail::to_string_view(fmt), args);
  size_t size = buffer.size();
  return rsl::fwrite(buffer.data(), sizeof(Char), size, f) < size ? -1 : static_cast<int>(size);
}
...
{
  using context = basic_printf_context_t<Char>;
  return vfprintf(f, detail::to_string_view(fmt), rsl::make_format_args<context>(args...));
}
...
{
  return vprintf(detail::to_string_view(fmt), rsl::make_format_args<basic_printf_context_t<char_t<S>>>(args...));
}
