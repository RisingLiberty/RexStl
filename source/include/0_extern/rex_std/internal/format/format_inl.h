...
namespace detail
{
    ...
    FMT_FUNC void assert_fail(const char8 *file, int32 line, const char8 *message)
    {
        // Use unchecked rsl::fprintf to avoid triggering another assertion when
        // writing to stderr fails
        rsl::fprintf(stderr, "%s:%d: assertion failed: %s", file, line, message);
        // Chosen instead of rsl::abort to satisfy Clang in CUDA mode during device
        // code pass.
        rsl::terminate();
    }

    FMT_FUNC void throw_format_error(const char8 *message)
    {
        FMT_THROW(format_error(message));
    }

    FMT_FUNC void format_error_code(detail::buffer<char8> &out, int32 error_code, string_view message) noexcept
    {
        // Report error code making sure that the output fits into
        // inline_buffer_size to avoid dynamic memory allocation and potential
        // bad_alloc.
        out.try_resize(0);
        static const char8 SEP[] = ": ";
        static const char8 ERROR_STR[] = "error ";
        // Subtract 2 to account for terminating null characters in SEP and ERROR_STR.
        size_t error_code_size = sizeof(SEP) + sizeof(ERROR_STR) - 2;
        auto abs_value = static_cast<uint32_or_64_or_128_t<int32>>(error_code);
        ...
        auto it = buffer_appender<char8>(out);
    ...
    FMT_FUNC void report_error(format_func func, int32 error_code, const char8 *message) noexcept
    {
        memory_buffer full_message;
        func(full_message, error_code, message);
        // Don't use fwrite_fully because the latter may throw.
        if (rsl::fwrite(full_message.data(), full_message.size(), 1, stderr) > 0)
            rsl::fputc('\n', stderr);
    }
    ...
    {
        size_t written = rsl::fwrite(ptr, size, count, stream);
        if (written < count)
    ...
    {
        static_assert(rsl::is_same_v<Locale, rsl::Locale>, "");
    }

    template <typename Locale> Locale locale_ref::get() const
    {
        static_assert(rsl::is_same_v<Locale, rsl::Locale>, "");
        return locale_ ? *static_cast<const rsl::Locale *>(locale_) : rsl::Locale();
    }

    template <typename Char> FMT_FUNC auto thousands_sep_impl(locale_ref loc) -> thousands_sep_result<Char>
    {
        auto &facet = rsl::use_facet<rsl::numpunct<Char>>(loc.get<rsl::Locale>());
        auto grouping = facet.grouping();
        auto thousands_sep = grouping.empty() ? Char() : facet.thousands_sep();
        return {rsl::move(grouping), thousands_sep};
    }
    template <typename Char> FMT_FUNC Char decimal_point_impl(locale_ref loc)
    {
        return rsl::use_facet<rsl::numpunct<Char>>(loc.get<rsl::Locale>()).decimal_point();
    }
#else
...
FMT_FUNC rsl::SystemError vsystem_error(int32 error_code, string_view format_str, format_args args)
{
    auto ec = rsl::ErrorCode(error_code, rsl::generic_category());
    return rsl::SystemError(ec, vformat(format_str, args));
}
...
namespace detail
{
    ...
    namespace dragonbox
    {
        ...
        inline int32 floor_log10_pow2(int32 e) noexcept
        {
        ...
        inline int32 floor_log2_pow10(int32 e) noexcept
        {
            FMT_ASSERT(e <= 1233 && e >= -1233, "too large exponent");
            return (e * 1741647) >> 19;
        }
        inline int32 floor_log10_pow2_minus_log10_4_over_3(int32 e) noexcept
        {
        ...
        template <int32 N> bool check_divisibility_and_divide_by_pow10(uint32_t &n) noexcept
        {
        ...
        // Precondition: n <= pow(10, N + 1).
        template <int32 N> uint32_t small_division_by_pow10(uint32_t n) noexcept
        {
            constexpr auto info = div_small_pow10_infos[N - 1];
            FMT_ASSERT(n <= info.divisor * 10, "n is too large");
            constexpr uint32_t magic_number = (1u << info.shift_amount) / info.divisor + 1;
        ...
        template <> struct cache_accessor<float>
        {
            ...
            static uint64_t get_cached_power(int32 k) noexcept
            {
            ...
            static uint32_t compute_delta(const cache_entry_type &cache, int32 beta) noexcept
            {
        ...
        template <> struct cache_accessor<double>
        {
            ...
            static uint128_fallback get_cached_power(int32 k) noexcept
            {
                ...
                static const int32 compression_ratio = 27;

                // Compute base index.
                int32 cache_index = (k - float_info<double>::min_k) / compression_ratio;
                int32 kb = cache_index * compression_ratio + float_info<double>::min_k;
                int32 offset = k - kb;
                ...
                int32 alpha = floor_log2_pow10(kb + offset) - floor_log2_pow10(kb) - offset;
...
static uint32_t compute_delta(cache_entry_type const &cache, int32 beta) noexcept
{
...
static carrier_uint compute_left_endpoint_for_shorter_interval_case(const cache_entry_type &cache, int32 beta) noexcept
{
    return (cache.high() - (cache.high() >> (num_significand_bits<double>() + 2))) >>
           (64 - num_significand_bits<double>() - 1 - beta);
}

static carrier_uint compute_right_endpoint_for_shorter_interval_case(const cache_entry_type &cache, int32 beta) noexcept
{
    return (cache.high() + (cache.high() >> (num_significand_bits<double>() + 1))) >>
           (64 - num_significand_bits<double>() - 1 - beta);
}

static carrier_uint compute_round_up_for_shorter_interval_case(const cache_entry_type &cache, int32 beta) noexcept
{
...
template <class T> bool is_left_endpoint_integer_shorter_interval(int32 exponent) noexcept
{
    const int32 case_shorter_interval_left_endpoint_lower_threshold = 2;
    const int32 case_shorter_interval_left_endpoint_upper_threshold = 3;
    return exponent >= case_shorter_interval_left_endpoint_lower_threshold &&
           exponent <= case_shorter_interval_left_endpoint_upper_threshold;
}

// Remove trailing zeros from n and return the number of zeros removed (float)
FMT_INLINE int32 remove_trailing_zeros(uint32_t &n) noexcept
{
    FMT_ASSERT(n != 0, "");
...
FMT_INLINE int32 remove_trailing_zeros(uint64_t &n) noexcept
{
    FMT_ASSERT(n != 0, "");
...
template <class T> FMT_INLINE decimal_fp<T> shorter_interval_case(int32 exponent) noexcept
{
    decimal_fp<T> ret_value;
    // Compute k and beta
    const int32 minus_k = floor_log10_pow2_minus_log10_4_over_3(exponent);
    const int32 beta = exponent + floor_log2_pow10(-minus_k);
...
template <typename T> decimal_fp<T> to_decimal(T x) noexcept
{
    ...
    int32 exponent = static_cast<int32>((br & exponent_mask<T>()) >> num_significand_bits<T>());
    ...
    // Compute k and beta.
    const int32 minus_k = floor_log10_pow2(exponent) - float_info<T>::kappa;
    const cache_entry_type cache = cache_accessor<T>::get_cached_power(-minus_k);
    const int32 beta = exponent + floor_log2_pow10(-minus_k);
...
#ifdef _MSC_VER
FMT_FUNC auto fmt_snprintf(char8 *buf, size_t size, const char8 *fmt, ...) -> int32
{
    auto args = va_list();
    va_start(args, fmt);
    int32 result = vsnprintf_s(buf, size, _TRUNCATE, fmt, args);
    va_end(args);
    return result;
}
#endif
...
FMT_FUNC detail::utf8_to_utf16::utf8_to_utf16(string_view s)
{
    for_each_codepoint(s, [this](uint32_t cp, string_view) {
        if (cp == invalid_code_point)
            FMT_THROW(rsl::RuntimeError("invalid utf8"));
        if (cp <= 0xFFFF)
        {
...
FMT_FUNC void format_system_error(detail::buffer<char8> &out, int32 error_code, const char8 *message) noexcept
{
    FMT_TRY
    {
        auto ec = rsl::ErrorCode(error_code, rsl::generic_category());
        write(rsl::back_inserter(out), rsl::SystemError(ec, message).what());
        return;
    }
    FMT_CATCH(...)
    {
...
FMT_FUNC void report_system_error(int32 error_code, const char8 *message) noexcept
{
...
namespace detail
{
    ...
    extern "C" __declspec(dllimport) int32 __stdcall WriteConsoleW( //
...
namespace detail
{
    FMT_FUNC void print(rsl::FILE *f, string_view text)
    {
#ifdef _WIN32
        auto fd = _fileno(f);
        if (_isatty(fd))
...
FMT_FUNC void vprint(rsl::FILE *f, string_view format_str, format_args args)
{
...
FMT_FUNC void detail::vprint_mojibake(rsl::FILE *f, string_view format_str, format_args args)
{
    memory_buffer buffer;
    detail::vformat_to(buffer, format_str, basic_format_args<buffer_context<char8>>(args));
...
namespace detail
{
    ...
    struct singleton
    {
        unsigned char8 upper;
        unsigned char8 lower_count;
    };

    inline auto is_printable(uint16_t x, const singleton *singletons, size_t singletons_size,
                             const unsigned char8 *singleton_lowers, const unsigned char8 *normal, size_t normal_size)
        -> bool
    {
        auto upper = x >> 8;
        auto lower_start = 0;
        ...
        auto xsigned = static_cast<int32>(x);
        auto current = true;
        for (size_t i = 0; i < normal_size; ++i)
        {
            auto v = static_cast<int32>(normal[i]);
    ...
    FMT_FUNC auto is_printable(uint32_t cp) -> bool
    {
        ...
        static constexpr unsigned char8 singletons0_lower[] = {
        ...
        static constexpr unsigned char8 singletons1_lower[] = {
        ...
        static constexpr unsigned char8 normal0[] = {
        ...
        static constexpr unsigned char8 normal1[] = {
