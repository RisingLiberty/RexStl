...
namespace rsl
{
    ...
    template <typename T>
    using IsNothrowCopyConstructibleStruct = is_nothrow_copy_constructible<T>;

    template <typename T>
    constexpr bool IsNothrowCopyConstructible = IsNothrowCopyConstructibleStruct<T>::value;
#endif
}