...
REX_RSL_BEGIN_NAMESPACE
    ...
    namespace internal
    {
        ...
        struct MakeUnsignedHelper0
        {
            ...
            using type = conditional_t<
                sizeof(T) <= sizeof(uint8), CharHelper,
                conditional_t<sizeof(T) <= sizeof(uint16), ShortHelper,
                conditional_t<sizeof(T) <= sizeof(uint32), IntHelper,
                conditional_t<sizeof(T) <= sizeof(uint64), LongLongHelper,
                conditional_t<sizeof(T) <= sizeof(ulong), LongHelper,
                NoTypeHelper>>>>>::type;
        };
    ...
    template <> struct make_unsigned<ulong>    { using type = ulong; };

    template <> struct make_unsigned<char>  { using type = uint8; };
    template <> struct make_unsigned<char8_t>  { using type = uint8; };
