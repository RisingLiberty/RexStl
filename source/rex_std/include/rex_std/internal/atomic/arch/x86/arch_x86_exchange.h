...
//
	...
	#define REX_ARCH_ATOMIC_X86_EXCHANGE_PRE_COMPUTE_DESIRED(ret, observed, val) \
		ret = (val)


	#define REX_ARCH_ATOMIC_EXCHANGE_RELAXED_64(type, ret, ptr, val)		\
		REX_ARCH_ATOMIC_X86_OP_64_IMPL(type, ret, ptr, val, RELAXED,		\
										 REX_ARCH_ATOMIC_X86_EXCHANGE_PRE_COMPUTE_DESIRED, \
										 REX_ARCH_ATOMIC_X86_NOP_POST_COMPUTE_RET)

	#define REX_ARCH_ATOMIC_EXCHANGE_ACQUIRE_64(type, ret, ptr, val)		\
		REX_ARCH_ATOMIC_X86_OP_64_IMPL(type, ret, ptr, val, ACQUIRE,		\
										 REX_ARCH_ATOMIC_X86_EXCHANGE_PRE_COMPUTE_DESIRED, \
										 REX_ARCH_ATOMIC_X86_NOP_POST_COMPUTE_RET)

	#define REX_ARCH_ATOMIC_EXCHANGE_RELEASE_64(type, ret, ptr, val)		\
		REX_ARCH_ATOMIC_X86_OP_64_IMPL(type, ret, ptr, val, RELEASE,		\
										 REX_ARCH_ATOMIC_X86_EXCHANGE_PRE_COMPUTE_DESIRED, \
										 REX_ARCH_ATOMIC_X86_NOP_POST_COMPUTE_RET)

	#define REX_ARCH_ATOMIC_EXCHANGE_ACQ_REL_64(type, ret, ptr, val)		\
		REX_ARCH_ATOMIC_X86_OP_64_IMPL(type, ret, ptr, val, ACQ_REL,		\
										 REX_ARCH_ATOMIC_X86_EXCHANGE_PRE_COMPUTE_DESIRED, \
										 REX_ARCH_ATOMIC_X86_NOP_POST_COMPUTE_RET)

	#define REX_ARCH_ATOMIC_EXCHANGE_SEQ_CST_64(type, ret, ptr, val)		\
	...
	#define REX_ARCH_ATOMIC_EXCHANGE_RELAXED_128(type, ret, ptr, val)		\
		REX_ARCH_ATOMIC_X86_EXCHANGE_128(type, ret, ptr, val, RELAXED)

	#define REX_ARCH_ATOMIC_EXCHANGE_ACQUIRE_128(type, ret, ptr, val)		\
		REX_ARCH_ATOMIC_X86_EXCHANGE_128(type, ret, ptr, val, ACQUIRE)

	#define REX_ARCH_ATOMIC_EXCHANGE_RELEASE_128(type, ret, ptr, val)		\
		REX_ARCH_ATOMIC_X86_EXCHANGE_128(type, ret, ptr, val, RELEASE)

	#define REX_ARCH_ATOMIC_EXCHANGE_ACQ_REL_128(type, ret, ptr, val)		\
		REX_ARCH_ATOMIC_X86_EXCHANGE_128(type, ret, ptr, val, ACQ_REL)

	#define REX_ARCH_ATOMIC_EXCHANGE_SEQ_CST_128(type, ret, ptr, val)		\
		REX_ARCH_ATOMIC_X86_EXCHANGE_128(type, ret, ptr, val, SEQ_CST)
