...
//
	 ...
	 */
		...
		#define REX_ARCH_ATOMIC_ARM32_LDREXD(ret, ptr)	\
			ret = __ldrexd((ptr))
	...
	#define REX_ARCH_ATOMIC_ARM_LOAD_8(type, ret, ptr)		\
		REX_ARCH_ATOMIC_ARM_LOAD_N(__int8, 8, type, ret, ptr)

	#define REX_ARCH_ATOMIC_ARM_LOAD_16(type, ret, ptr)		\
		REX_ARCH_ATOMIC_ARM_LOAD_N(__int16, 16, type, ret, ptr)

	#define REX_ARCH_ATOMIC_ARM_LOAD_32(type, ret, ptr)		\
		REX_ARCH_ATOMIC_ARM_LOAD_N(__int32, 32, type, ret, ptr)
		...
		#define REX_ARCH_ATOMIC_ARM_LOAD_64(type, ret, ptr)		\
			REX_ARCH_ATOMIC_ARM_LOAD_N(__int64, 64, type, ret, ptr)
	...
	#define REX_ARCH_ATOMIC_ARM_LOAD_128(type, ret, ptr, MemoryOrder)		\
		{																	\
			...
			do																\
			{																\
				MERGE(MERGE(REX_ATOMIC_CMPXCHG_STRONG_, MemoryOrder), _128)(type, cmpxchgRetBool, \
																											ptr, &(ret), ret); \
	...
	#define REX_ARCH_ATOMIC_LOAD_RELAXED_128(type, ret, ptr)	\
		REX_ARCH_ATOMIC_ARM_LOAD_128(type, ret, ptr, RELAXED)
	...
	#define REX_ARCH_ATOMIC_LOAD_ACQUIRE_128(type, ret, ptr)		\
		REX_ARCH_ATOMIC_ARM_LOAD_128(type, ret, ptr, ACQUIRE)
	...
	#define REX_ARCH_ATOMIC_LOAD_SEQ_CST_128(type, ret, ptr)		\
		REX_ARCH_ATOMIC_ARM_LOAD_128(type, ret, ptr, SEQ_CST)
