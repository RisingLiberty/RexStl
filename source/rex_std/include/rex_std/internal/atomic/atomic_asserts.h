...
#define REX_ATOMIC_STATIC_ASSERT_INVALID_MEMORY_ORDER(type)			\
	static_assert(!rsl::is_same<type, type>::value, "rsl::atomic<T> : invalid memory order for the given operation!");
...
#define REX_ATOMIC_STATIC_ASSERT_TYPE_IS_OBJECT(type) \
	static_assert(rsl::is_object<type>::value, "rsl::atomic<T> : Template Typename T must be an object type!");
