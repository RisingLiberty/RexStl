 ...
 */
static_assert(__atomic_always_lock_free(1, 0), "rsl::atomic<T> where sizeof(T) == 1 must be lock-free!");
 ...
 * We don't care about the read-only restriction because our rsl::atomic<T> object is mutable
 ...
 *
 * static_assert(__atomic_always_lock_free(16, 0), "rsl::atomic<T> where sizeof(T) == 16 must be lock-free!");
 */
