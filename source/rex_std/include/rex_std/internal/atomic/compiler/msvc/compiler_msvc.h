 ...
 */
	...
	#define REX_MSVC_ATOMIC_FETCH_OP(ret, ptr, val, MemoryOrder, Intrinsic) \
		ret = Intrinsic(ptr, val)

	#define REX_MSVC_ATOMIC_EXCHANGE_OP(ret, ptr, val, MemoryOrder, Intrinsic) \
		ret = Intrinsic(ptr, val)

	#define REX_MSVC_ATOMIC_CMPXCHG_STRONG_OP(ret, ptr, comparand, exchange, MemoryOrder, Intrinsic) \
		ret = Intrinsic(ptr, exchange, comparand)

	#define REX_MSVC_ATOMIC_CMPXCHG_STRONG_128_OP(ret, ptr, comparandResult, exchangeHigh, exchangeLow, MemoryOrder) \
	...
	#define REX_MSVC_INTRINSIC_RELAXED(Intrinsic) \
		MERGE(Intrinsic, _nf)

	#define REX_MSVC_INTRINSIC_ACQUIRE(Intrinsic)	\
		MERGE(Intrinsic, _acq)

	#define REX_MSVC_INTRINSIC_RELEASE(Intrinsic)	\
		MERGE(Intrinsic, _rel)

	#define REX_MSVC_INTRINSIC_ACQ_REL(Intrinsic)	\
		Intrinsic

	#define REX_MSVC_INTRINSIC_SEQ_CST(Intrinsic)	\
		Intrinsic


	#define REX_MSVC_ATOMIC_FETCH_OP(ret, ptr, val, MemoryOrder, Intrinsic) \
		ret = MERGE(REX_MSVC_INTRINSIC_, MemoryOrder)(Intrinsic)(ptr, val)

	#define REX_MSVC_ATOMIC_EXCHANGE_OP(ret, ptr, val, MemoryOrder, Intrinsic) \
		ret = MERGE(REX_MSVC_INTRINSIC_, MemoryOrder)(Intrinsic)(ptr, val)

	#define REX_MSVC_ATOMIC_CMPXCHG_STRONG_OP(ret, ptr, comparand, exchange, MemoryOrder, Intrinsic) \
		ret = MERGE(REX_MSVC_INTRINSIC_, MemoryOrder)(Intrinsic)(ptr, exchange, comparand)

	#define REX_MSVC_ATOMIC_CMPXCHG_STRONG_128_OP(ret, ptr, comparandResult, exchangeHigh, exchangeLow, MemoryOrder) \
		ret = MERGE(REX_MSVC_INTRINSIC_, MemoryOrder)(_InterlockedCompareExchange128)(ptr, exchangeHigh, exchangeLow, comparandResult)
...
#define REX_MSVC_NOP_PRE_INTRIN_COMPUTE(ret, val) \
	ret = (val)
...
#define REX_MSVC_ATOMIC_FETCH_INTRIN_N(integralType, fetchIntrinsic, type, ret, ptr, val, MemoryOrder, PRE_INTRIN_COMPUTE, POST_INTRIN_COMPUTE) \
	{																	\
		...
		const integralType valIntegral = REX_ATOMIC_TYPE_PUN_CAST(integralType, valCompute); \
																		\
		REX_MSVC_ATOMIC_FETCH_OP(retIntegral, REX_ATOMIC_VOLATILE_INTEGRAL_CAST(integralType, (ptr)), \
...
#define REX_MSVC_ATOMIC_EXCHANGE_INTRIN_N(integralType, exchangeIntrinsic, type, ret, ptr, val, MemoryOrder) \
	{																	\
		integralType retIntegral;										\
		REX_MSVC_ATOMIC_EXCHANGE_OP(retIntegral, REX_ATOMIC_VOLATILE_INTEGRAL_CAST(integralType, (ptr)), \
									  REX_ATOMIC_TYPE_PUN_CAST(integralType, (val)), MemoryOrder, \
									  exchangeIntrinsic);				\
																		\
		ret = REX_ATOMIC_TYPE_PUN_CAST(type, retIntegral);			\
	}
...
#define REX_MSVC_ATOMIC_CMPXCHG_STRONG_INTRIN_N(integralType, cmpxchgStrongIntrinsic, type, ret, ptr, expected, desired, MemoryOrder) \
	{																	\
		...
		REX_MSVC_ATOMIC_CMPXCHG_STRONG_OP(oldIntegral, REX_ATOMIC_VOLATILE_INTEGRAL_CAST(integralType, (ptr)), \
											comparandIntegral, REX_ATOMIC_TYPE_PUN_CAST(integralType, (desired)), \
...
#define REX_MSVC_ATOMIC_CMPXCHG_STRONG_INTRIN_128(type, ret, ptr, expected, desired, MemoryOrder) \
	{																	\
		...
		cmpxchgRetChar = REX_MSVC_ATOMIC_CMPXCHG_STRONG_128_OP(cmpxchgRetChar, REX_ATOMIC_VOLATILE_TYPE_CAST(__int64, (ptr)), \
																 REX_ATOMIC_TYPE_CAST(__int64, (expected)), \
...
#define REX_MSVC_ATOMIC_EXCHANGE_OP_N(integralType, exchangeIntrinsic, type, ret, ptr, val, MemoryOrder) \
	REX_MSVC_ATOMIC_EXCHANGE_INTRIN_N(integralType, exchangeIntrinsic, type, ret, ptr, val, MemoryOrder)
...
#define REX_MSVC_ATOMIC_CMPXCHG_STRONG_OP_128(type, ret, ptr, expected, desired, MemoryOrder) \
	REX_MSVC_ATOMIC_CMPXCHG_STRONG_INTRIN_128(type, ret, ptr, expected, desired, MemoryOrder)
