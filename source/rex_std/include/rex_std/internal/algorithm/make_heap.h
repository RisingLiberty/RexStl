    ...
    void make_heap(RandomAccessIterator first, RandomAccessIterator last)
    {
      ...
      if(heapSize >= 2) // If there is anything to do... (we need this check because otherwise the math fails below).
      {
        ...
        do
        {
          ...
          rsl::adjust_heap<RandomAccessIterator, difference_type, value_type>(first, parentPosition, heapSize, parentPosition, rsl::forward<value_type>(temp));
    ...
    void make_heap(RandomAccessIterator first, RandomAccessIterator last, Compare compare)
    {
      ...
      if(heapSize >= 2) // If there is anything to do... (we need this check because otherwise the math fails below).
      {
        ...
        do
        {
          --parentPosition;
          value_type temp(rsl::forward<value_type>(*(first + parentPosition)));
          rsl::adjust_heap<RandomAccessIterator, difference_type, value_type, Compare>(first, parentPosition, heapSize, parentPosition, rsl::forward<value_type>(temp), compare);
        } while(parentPosition != 0);
      }
    }
  } // namespace v1
