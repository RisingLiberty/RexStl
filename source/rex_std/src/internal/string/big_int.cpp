...
namespace rsl
{
    ...
    // - <windows sdk>/ucrt/inc/corecrt_internal_strtox.h
      ...
      big_int __cdecl make_big_int_power_of_two(uint32 power)
      {
        ...
        memset(x.data, 0, element_index * sizeof(uint32));
      ...
      uint32 __cdecl bit_scan_reverse(uint64 value)
      {
        if (value > UINT32_MAX)
        {
          return bit_scan_reverse(reinterpret_cast<const uint32*>(&value)[1]) + 32;
        }
        else
        {
          return bit_scan_reverse(reinterpret_cast<const uint32*>(&value)[0]);
        }
      }
      ...
      )
      {
        ...
        u_add = static_cast<uint32>(uu_res);
      ...
      bool __cdecl multiply_by_power_of_ten(big_int& x, const uint32 power)
      {
        ...
        static const uint32 large_power_data[] =
        {
        ...
        static const uint32 small_powers_of_ten[9] =
        {
              10,
             100,
            1000,
