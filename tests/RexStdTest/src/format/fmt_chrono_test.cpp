...
TEST_CASE("chrono_test, format_tm")
{
  ...
  for (auto ctm : tm_list)
  {
    ...
    REQUIRE(system_strftime(iso_week_spec, &tm),
      rsl::format(rsl::runtime(fmt_spec), tm));
  }
...
TEST_CASE("chrono_test, system_clock_time_point") {
  ...
  REQUIRE(strftime_full_utc(t1), rsl::format("{:%Y-%m-%d %H:%M:%S}", t1));
  REQUIRE(strftime_full_utc(t1), rsl::format("{}", t1));
  using time_point =
    rsl::chrono::time_point<rsl::chrono::system_clock, rsl::chrono::seconds>;
  auto t2 = time_point(rsl::chrono::seconds(42));
  REQUIRE(strftime_full_utc(t2), rsl::format("{:%Y-%m-%d %H:%M:%S}", t2));
  ...
  if (rsl::detail::has_member_data_tm_gmtoff<rsl::tm>::value) {
    ...
    REQUIRE("+0000", rsl::format("{:%z}", t1));
    REQUIRE("+0000", rsl::format("{:%z}", tm));

    REQUIRE("+00:00", rsl::format("{:%Ez}", t1));
    REQUIRE("+00:00", rsl::format("{:%Ez}", tm));

    REQUIRE("+00:00", rsl::format("{:%Oz}", t1));
    REQUIRE("+00:00", rsl::format("{:%Oz}", tm));
  }
}
...
{
  REQUIRE("42s", rsl::format("{}", rsl::chrono::seconds(42)));
  REQUIRE("42as",
  ...
  REQUIRE("42ns", rsl::format("{}", rsl::chrono::nanoseconds(42)));
  REQUIRE("42µs", rsl::format("{}", rsl::chrono::microseconds(42)));
  REQUIRE("42ms", rsl::format("{}", rsl::chrono::milliseconds(42)));
  REQUIRE("42cs",
  ...
  REQUIRE("42m", rsl::format("{}", rsl::chrono::minutes(42)));
  REQUIRE("42h", rsl::format("{}", rsl::chrono::hours(42)));
  REQUIRE(
    "42[15]s",
...
TEST_CASE("chrono_test, align") {
  ...
  REQUIRE("42s  ", rsl::format("{:5}", s));
  REQUIRE("42s  ", rsl::format("{:{}}", s, 5));
  REQUIRE("  42s", rsl::format("{:>5}", s));
  REQUIRE("**42s**", rsl::format("{:*^7}", s));
  REQUIRE("03:25:45    ",
...
TEST_CASE("chrono_test, format_specs") {
  REQUIRE("%", rsl::format("{:%%}", rsl::chrono::seconds(0)));
  REQUIRE("\n", rsl::format("{:%n}", rsl::chrono::seconds(0)));
  REQUIRE("\t", rsl::format("{:%t}", rsl::chrono::seconds(0)));
  REQUIRE("00", rsl::format("{:%S}", rsl::chrono::seconds(0)));
  REQUIRE("00", rsl::format("{:%S}", rsl::chrono::seconds(60)));
  REQUIRE("42", rsl::format("{:%S}", rsl::chrono::seconds(42)));
  REQUIRE("01.234", rsl::format("{:%S}", rsl::chrono::milliseconds(1234)));
  REQUIRE("00", rsl::format("{:%M}", rsl::chrono::minutes(0)));
  REQUIRE("00", rsl::format("{:%M}", rsl::chrono::minutes(60)));
  REQUIRE("42", rsl::format("{:%M}", rsl::chrono::minutes(42)));
  REQUIRE("01", rsl::format("{:%M}", rsl::chrono::seconds(61)));
  REQUIRE("00", rsl::format("{:%H}", rsl::chrono::hours(0)));
  REQUIRE("00", rsl::format("{:%H}", rsl::chrono::hours(24)));
  REQUIRE("14", rsl::format("{:%H}", rsl::chrono::hours(14)));
  REQUIRE("01", rsl::format("{:%H}", rsl::chrono::minutes(61)));
  REQUIRE("12", rsl::format("{:%I}", rsl::chrono::hours(0)));
  REQUIRE("12", rsl::format("{:%I}", rsl::chrono::hours(12)));
  REQUIRE("12", rsl::format("{:%I}", rsl::chrono::hours(24)));
  REQUIRE("04", rsl::format("{:%I}", rsl::chrono::hours(4)));
  REQUIRE("02", rsl::format("{:%I}", rsl::chrono::hours(14)));
  REQUIRE("03:25:45",
    rsl::format("{:%H:%M:%S}", rsl::chrono::seconds(12345)));
  REQUIRE("03:25", rsl::format("{:%R}", rsl::chrono::seconds(12345)));
  REQUIRE("03:25:45", rsl::format("{:%T}", rsl::chrono::seconds(12345)));
  REQUIRE("12345", rsl::format("{:%Q}", rsl::chrono::seconds(12345)));
  REQUIRE("s", rsl::format("{:%q}", rsl::chrono::seconds(12345)));
}
...
auto format_tm(const rsl::tm& time, rsl::string_view spec,
  const rsl::locale& loc) -> rsl::string {
  auto& facet = rsl::use_facet<rsl::time_put<char>>(loc);
  rsl::ostringstream os;
  os.imbue(loc);
...
TEST_CASE("chrono_test, format_default_fp") {
  ...
  REQUIRE("1.234s", rsl::format("{}", fs(1.234)));
  typedef rsl::chrono::duration<float, rsl::milli> fms;
  REQUIRE("1.234ms", rsl::format("{}", fms(1.234)));
  typedef rsl::chrono::duration<double> ds;
  REQUIRE("1.234s", rsl::format("{}", ds(1.234)));
  REQUIRE("1.234ms", rsl::format("{}", dms(1.234)));
}
...
TEST_CASE("chrono_test, format_precision") {
  ...
  REQUIRE("1ms", rsl::format("{:.0}", dms(1.234)));
  REQUIRE("1.2ms", rsl::format("{:.1}", dms(1.234)));
  REQUIRE("1.23ms", rsl::format("{:.{}}", dms(1.234), 2));

  REQUIRE("13ms", rsl::format("{:.0}", dms(12.56)));
  REQUIRE("12.6ms", rsl::format("{:.1}", dms(12.56)));
  REQUIRE("12.56ms", rsl::format("{:.2}", dms(12.56)));
}

TEST_CASE("chrono_test, format_full_specs") {
  REQUIRE("1ms   ", rsl::format("{:6.0}", dms(1.234)));
  REQUIRE("1.2ms ", rsl::format("{:6.1}", dms(1.234)));
  REQUIRE("  1.23ms", rsl::format("{:>8.{}}", dms(1.234), 2));
  REQUIRE(" 1.2ms ", rsl::format("{:^{}.{}}", dms(1.234), 7, 1));
  REQUIRE(" 1.23ms ", rsl::format("{0:^{2}.{1}}", dms(1.234), 2, 8));
  REQUIRE("=1.234ms=", rsl::format("{:=^{}.{}}", dms(1.234), 9, 3));
  REQUIRE("*1.2340ms*", rsl::format("{:*^10.4}", dms(1.234)));

  REQUIRE("13ms  ", rsl::format("{:6.0}", dms(12.56)));
  REQUIRE("    13ms", rsl::format("{:>8.{}}", dms(12.56), 0));
  REQUIRE(" 13ms ", rsl::format("{:^{}.{}}", dms(12.56), 6, 0));
  REQUIRE("  13ms  ", rsl::format("{0:^{2}.{1}}", dms(12.56), 0, 8));
  REQUIRE("==13ms===", rsl::format("{:=^{}.{}}", dms(12.56), 9, 0));
  REQUIRE("***13ms***", rsl::format("{:*^10.0}", dms(12.56)));
}

TEST_CASE("chrono_test, format_simple_q") {
  typedef rsl::chrono::duration<float> fs;
  REQUIRE("1.234 s", rsl::format("{:%Q %q}", fs(1.234)));
  typedef rsl::chrono::duration<float, rsl::milli> fms;
  REQUIRE("1.234 ms", rsl::format("{:%Q %q}", fms(1.234)));
  typedef rsl::chrono::duration<double> ds;
  REQUIRE("1.234 s", rsl::format("{:%Q %q}", ds(1.234)));
  REQUIRE("1.234 ms", rsl::format("{:%Q %q}", dms(1.234)));
}
...
TEST_CASE("chrono_test, format_precision_q") {
  EXPECT_THROW_MSG(
    ...
    rsl::format_error, "precision not allowed for this argument type");
  REQUIRE("1.2 ms", rsl::format("{:.1%Q %q}", dms(1.234)));
  REQUIRE("1.23 ms", rsl::format("{:.{}%Q %q}", dms(1.234), 2));
}

TEST_CASE("chrono_test, format_full_specs_q") {
  REQUIRE("1 ms   ", rsl::format("{:7.0%Q %q}", dms(1.234)));
  REQUIRE("1.2 ms ", rsl::format("{:7.1%Q %q}", dms(1.234)));
  REQUIRE(" 1.23 ms", rsl::format("{:>8.{}%Q %q}", dms(1.234), 2));
  REQUIRE(" 1.2 ms ", rsl::format("{:^{}.{}%Q %q}", dms(1.234), 8, 1));
  REQUIRE(" 1.23 ms ", rsl::format("{0:^{2}.{1}%Q %q}", dms(1.234), 2, 9));
  REQUIRE("=1.234 ms=", rsl::format("{:=^{}.{}%Q %q}", dms(1.234), 10, 3));
  REQUIRE("*1.2340 ms*", rsl::format("{:*^11.4%Q %q}", dms(1.234)));

  REQUIRE("13 ms  ", rsl::format("{:7.0%Q %q}", dms(12.56)));
  REQUIRE("   13 ms", rsl::format("{:>8.{}%Q %q}", dms(12.56), 0));
  REQUIRE(" 13 ms  ", rsl::format("{:^{}.{}%Q %q}", dms(12.56), 8, 0));
  REQUIRE("  13 ms  ", rsl::format("{0:^{2}.{1}%Q %q}", dms(12.56), 0, 9));
  REQUIRE("==13 ms==", rsl::format("{:=^{}.{}%Q %q}", dms(12.56), 9, 0));
  REQUIRE("***13 ms***", rsl::format("{:*^11.0%Q %q}", dms(12.56)));
}
...
TEST_CASE("chrono_test, special_durations") {
  ...
  REQUIRE(rsl::format("{}", rsl::chrono::duration<float, rsl::exa>(1)),
    "1Es");
  REQUIRE(rsl::format("{}", rsl::chrono::duration<float, rsl::atto>(1)),
    "1as");
  REQUIRE(rsl::format("{:%R}", rsl::chrono::duration<char, rsl::mega>{2}),
    "03:33");
  REQUIRE(rsl::format("{:%T}", rsl::chrono::duration<char, rsl::mega>{2}),
    "03:33:20");
  REQUIRE("44.000000000000",
...
TEST_CASE("chrono_test, weekday") {
  ...
  REQUIRE(rsl::format("{}", mon), "Mon");
  REQUIRE(rsl::format("{:%a}", tm), "Mon");
...
TEST_CASE("chrono_test, cpp20_duration_subsecond_support") {
  ...
  REQUIRE(rsl::format("{:%S}", attoseconds{ 999999999999999999 }),
    "00.999999999999999999");
  REQUIRE(rsl::format("{:%S}", attoseconds{ 673231113420148734 }),
    "00.673231113420148734");
  REQUIRE(rsl::format("{:%S}", attoseconds{ -673231113420148734 }),
    "-00.673231113420148734");
  REQUIRE(rsl::format("{:%S}", rsl::chrono::nanoseconds{ 13420148734 }),
    "13.420148734");
  REQUIRE(rsl::format("{:%S}", rsl::chrono::nanoseconds{ -13420148734 }),
    "-13.420148734");
  REQUIRE(rsl::format("{:%S}", rsl::chrono::milliseconds{ 1234 }), "01.234");
  // Check subsecond presision modifier.
  REQUIRE(rsl::format("{:.6%S}", rsl::chrono::nanoseconds{ 1234 }),
    "00.000001");
  REQUIRE(rsl::format("{:.18%S}", rsl::chrono::nanoseconds{ 1234 }),
    "00.000001234000000000");
  REQUIRE(rsl::format("{:.{}%S}", rsl::chrono::nanoseconds{ 1234 }, 6),
    "00.000001");
  REQUIRE(rsl::format("{:.6%S}", rsl::chrono::milliseconds{ 1234 }),
    "01.234000");
  REQUIRE(rsl::format("{:.6%S}", rsl::chrono::milliseconds{ -1234 }),
    "-01.234000");
  REQUIRE(rsl::format("{:.3%S}", rsl::chrono::seconds{ 1234 }), "34.000");
  REQUIRE(rsl::format("{:.3%S}", rsl::chrono::hours{ 1234 }), "00.000");
  REQUIRE(rsl::format("{:.5%S}", dms(1.234)), "00.00123");
  REQUIRE(rsl::format("{:.8%S}", dms(1.234)), "00.00123400");
  {
    // Check that {:%H:%M:%S} is equivalent to {:%T}.
    auto dur = rsl::chrono::milliseconds{ 3601234 };
    auto formatted_dur = rsl::format("{:%T}", dur);
    REQUIRE(formatted_dur, "01:00:01.234");
    REQUIRE(rsl::format("{:%H:%M:%S}", dur), formatted_dur);
    REQUIRE(rsl::format("{:.6%H:%M:%S}", dur), "01:00:01.234000");
  }
  using nanoseconds_dbl = rsl::chrono::duration<double, rsl::nano>;
  REQUIRE(rsl::format("{:%S}", nanoseconds_dbl{ -123456789 }), "-00.123456789");
  REQUIRE(rsl::format("{:%S}", nanoseconds_dbl{ 9123456789 }), "09.123456789");
  // Verify that only the seconds part is extracted and printed.
  REQUIRE(rsl::format("{:%S}", nanoseconds_dbl{ 99123456789 }), "39.123456789");
  REQUIRE(rsl::format("{:%S}", nanoseconds_dbl{ 99123000000 }), "39.123000000");
  {
    // Now the hour is printed, and we also test if negative doubles work.
    auto dur = nanoseconds_dbl{ -99123456789 };
    auto formatted_dur = rsl::format("{:%T}", dur);
    REQUIRE(formatted_dur, "-00:01:39.123456789");
    REQUIRE(rsl::format("{:%H:%M:%S}", dur), formatted_dur);
    REQUIRE(rsl::format("{:.3%H:%M:%S}", dur), "-00:01:39.123");
  }
  // Check that durations with precision greater than rsl::chrono::seconds have
  // fixed precision, and print zeros even if there is no fractional part.
  REQUIRE(rsl::format("{:%S}", rsl::chrono::microseconds{ 7000000 }),
    "07.000000");
  REQUIRE(rsl::format("{:%S}",
  ...
  REQUIRE(rsl::format("{:%S}", rsl::chrono::duration<double>{1.5}),
    "01.500000");
  REQUIRE(rsl::format("{:%M:%S}", rsl::chrono::duration<double>{-61.25}),
    "-01:01.250000");
}
...
TEST_CASE("chrono_test, timestamps_sub_seconds") {
  ...
  REQUIRE(rsl::format("{}.{}", strftime_full_utc(t9_sec), t9_sub_sec_part),
  ...
  REQUIRE(rsl::format("{:%S}", t10), "02.000");
}
